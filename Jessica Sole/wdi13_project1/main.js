...
var p1RowWin = function() {
    ...
    if (p1Row0Total === 3) {
        p1DanceWinner(); // adds ninja dance gif and winner css to the win div
        changeP1Score();
...
var p2RowWin = function() {
    ...
    //determins which has 3 in a row for across winner and calls a function to display who won
    if (p2Row0Total === 3) {
        p2CatWinner();// adds cat ninja gif and winner css to the win div
        changeP2Score();
...
var p1ColWin = function() {
    ...
    if (p1Col0Total === 3) {
        p1LegoNinjaWin();  //adds lego ninja gif to win div
        changeP1Score();
...
var p2ColWin = function() {
    ...
    //determins which has 3 in a row for column winner and calls a function to display who won
    if (p2Col0Total === 3) {
        p2HandNinjaWin();  //adds hand ninja gif to win div
         changeP2Score();
...
var p1DanceWinner = function() {
    winnerDiv.innerHTML += "Player One Wins"
    winnerDiv.classList.toggle("winner-p1-across");
    restartDiv.style.zIndex = "0";
    gameBoard.style.zIndex = "-1";
}

var p2CatWinner = function() {
    winnerDiv.innerHTML += "Player Two Wins"
    winnerDiv.classList.toggle("winner-p2-across");
    gameBoard.style.zIndex = "-1";
    restartDiv.style.zIndex = "0";
}

var p1LegoNinjaWin = function() {
    winnerDiv.innerHTML += "Player One Wins"
    winnerDiv.classList.toggle("lego-win");
    gameBoard.style.zIndex = "-1";
    restartDiv.style.zIndex = "0";

}

var p2HandNinjaWin = function() {
    winnerDiv.innerHTML += "Player Two Wins"
...
var p1KKidWin = function() {
    winnerDiv.innerHTML += "Player One Wins"
    winnerDiv.classList.toggle("k-kid");
    gameBoard.style.zIndex = "-1";
    restartDiv.style.zIndex = "0";

}

var p2CatLaughWin = function() {
    winnerDiv.innerHTML += "Player Two Wins"
    winnerDiv.classList.toggle("cat-laugh");
    gameBoard.style.zIndex = "-1";
    restartDiv.style.zIndex = "0";
...
var resetPlayerBoard = function() {
    newWinnerDiv.addEventListener("click", function() {
        winnerDiv.classList.remove("winner-p1-across");
        //p2BoardSquares.classList.remove("ninja-cat");
        //p1BoardSquares.classList.remove("ninja-dog");
        winnerDiv.innerHTML += " "
        restartDiv.style.zIndex = "-1";
        gameBoard.style.zIndex = "0";

        for (var e =0; e<=3; e++){
            if (p1BoardSquares[e].classList[2] === "ninja-cat" ){
                   p2BoardSquares[e].classList[2].remove("ninja-cat");
                }else if (p1BoardSquares[e].classList[2] === "ninja-dog" ){
                   p1BoardSquares[e].classList[2].remove("ninja-dog");
                }
            }
